...
/**
 * @see \Symfony\Component\Mime\Part\Multipart\FormDataPart
 */
final class FormDataPart extends AbstractMultipartPart
{
    ...
    private function prepareFields(array $fields): array
    {
        ...
        $prepare = function ($item, $key, $root = null) use (&$values, &$prepare) {
            ...
            // Freshdesk does not accept "attachments[0]", "attachments[1]" but only multiple "attachments[]"
            $fieldName = null !== $root ? sprintf('%s[%s]', $root, is_numeric($key) ? '' : $key) : $key;
